...
# print(aims.aims)
x_train, x_test, y_train, y_test = train_test_split(test_train_data,aims.aims,test_size=0.15)
...
model.add(Dense(units=8,activation='relu',name="Dense_2"))#activation='relu'
model.add(Dropout(0.2))
model.add(Dense(units=2,activation='softmax',name="Dense_3"))
...
model.compile(loss='binary_crossentropy',#'binary_crossentropy''categorical_crossentropy'
        ...
        metrics=['accuracy'])
#模型估计
model.fit(x_train, y_train, epochs=250, batch_size=25,callbacks=callback_lists)#verbose='True',callbacks=callback_lists)# callbacks=[history])
